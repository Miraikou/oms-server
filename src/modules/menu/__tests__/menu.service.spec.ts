...
describe('MenuService', () => {
  ...
  describe('findAll', () => {
    it('应返回不含按钮的菜单树', async () => {
      const menus = [
        { id: '1', parentId: null, menuName: '系统管理', menuType: 0, sortNo: 1, status: 1, createdTime: new Date() },
        { id: '2', parentId: '1', menuName: '用户管理', menuType: 1, sortNo: 1, status: 1, createdTime: new Date() },
      ]
  ...
  describe('update', () => {
    it('应成功更新菜单', async () => {
      mockMenuRepo.findOne.mockResolvedValue({
        id: '1', menuName: '旧名称', menuType: 1, parentId: null,
        permission: null, path: '/old', component: null, icon: null,
        sortNo: 1, visible: 1, status: 1,
      })
    ...
    it('菜单不存在应抛出 NotFoundException', async () => {
      ...
      await expect(service.update('999', { menuName: 'test' }))
  ...
  describe('findUserPermissions', () => {
    ...
    it('应返回用户有权限的菜单树和按钮权限列表', async () => {
      ...
      mockMenuRepo.createQueryBuilder.mockReturnValue({
        ...
        addOrderBy: jest.fn().mockReturnThis(),
        getMany: jest.fn().mockResolvedValue([
          { id: 'dir-1', parentId: null, menuName: '订单管理', menuType: 0, sortNo: 1, status: 1, visible: 1, permission: null },
          { id: 'menu-1', parentId: 'dir-1', menuName: '订单列表', menuType: 1, sortNo: 1, status: 1, visible: 1, path: '/orders', permission: null },
          { id: 'btn-1', parentId: 'menu-1', menuName: '新增', menuType: 2, sortNo: 1, status: 1, visible: 1, permission: 'order:create' },
        ]),
      })
