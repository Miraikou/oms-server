...
export class HelpDocService extends BaseCrudService<HelpDoc> {
	...
	async findAll(query: QueryHelpDocDto): Promise<{
		list: HelpDoc[];
		total: number;
		page: number;
		pageSize: number;
		...
		if (query.keyword) {
			qb.andWhere(`${this.alias}.title LIKE :kw`, {
				kw: `%${query.keyword}%`,
			});
		}
		if (query.category) {
	...
	/** 获取文档分类列表（去重，按分类内最小排序号升序） */
	async getCategory(): Promise<string[]> {
		const rows = await this.repo
			.createQueryBuilder(this.alias)
			.select(`${this.alias}.category`, 'category')
			.addSelect(`MIN(${this.alias}.sortOrder)`, 'min_sort')
			.groupBy(`${this.alias}.category`)
			.orderBy('min_sort', 'ASC')
			.getRawMany();
		return rows.map((it) => it.category);
	}
}
