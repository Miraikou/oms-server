...
describe('RoleService', () => {
  ...
  describe('update', () => {
    ...
    it('角色不存在应抛出 NotFoundException', async () => {
      ...
      await expect(service.update('999', { roleName: 'test' })).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('assignMenus', () => {
    it('应在事务中先删后插', async () => {
      ...
      expect(mockManager.delete).toHaveBeenCalledWith(SysRoleMenu, {
        roleId: '1',
      });
    ...
    it('角色不存在应抛出 NotFoundException', async () => {
      ...
      await expect(service.assignMenus('999', [])).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('findUserRoleCodes', () => {
    it('应返回用户的角色编码列表', async () => {
      ...
      mockRoleRepo.createQueryBuilder.mockReturnValue({
        ...
        getMany: jest
          .fn()
          .mockResolvedValue([{ roleCode: 'SALES' }, { roleCode: 'FINANCE' }]),
      } as any);
