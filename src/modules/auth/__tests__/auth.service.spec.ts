...
describe('AuthService', () => {
  ...
  const mockMenuService = {
    ...
    findUserPermissions: jest.fn().mockResolvedValue({ menus: [], permissions: [] }),
  }
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(SysLoginLog), useValue: mockLoginLogRepo },
  ...
  describe('login', () => {
    ...
    it('登录成功应记录登录日志', async () => {
      ...
      await service.login({ username: 'admin', password: 'admin123' }, '127.0.0.1', 'UA')
  ...
  describe('refreshToken', () => {
    it('有效 refreshToken 应返回新 accessToken', async () => {
      mockJwtService.verify.mockReturnValue({ sub: '1000000000000001', username: 'admin' })
      mockUserRepo.findOne.mockResolvedValue(testUser)
  ...
  describe('getUserMenus', () => {
    it('SUPER_ADMIN 应返回全部菜单和权限', async () => {
      ...
      mockMenuService.findAll.mockResolvedValue([{ id: '1', menuName: '驾驶舱' }])
      mockMenuService.findAllPermissions.mockResolvedValue(['order:create', 'order:edit'])
