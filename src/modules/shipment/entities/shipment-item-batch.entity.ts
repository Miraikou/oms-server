...
export class ShipmentItemBatch {
  ...
  @Column({ name: 'inventory_batch_id', type: 'bigint', comment: '库存批次 ID' })
  inventoryBatchId: string

  @Column({ type: 'decimal', precision: 18, scale: 4, comment: '消耗数量' })
  quantity: string

  @Column({ name: 'unit_cost', type: 'decimal', precision: 18, scale: 2, comment: '批次成本单价' })
  unitCost: string

  @Column({ name: 'total_cost', type: 'decimal', precision: 18, scale: 2, comment: '成本金额' })
  totalCost: string
