 ...
 */
@Entity('shipment')
@Index('uk_express_tracking', ['expressCompanyId', 'trackingNo'], { unique: true })
export class Shipment extends BaseEntity {
  @Index('uk_shipment_no', { unique: true })
  @Column({ name: 'shipment_no', type: 'varchar', length: 50, comment: '发货单号' })
  shipmentNo: string
  ...
  @Column({ name: 'express_company_id', type: 'bigint', comment: '快递公司 ID' })
  expressCompanyId: string

  @Index('idx_tracking_no')
  @Column({ name: 'tracking_no', type: 'varchar', length: 100, comment: '快递单号' })
  trackingNo: string
