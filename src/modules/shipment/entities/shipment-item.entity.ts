...
export class ShipmentItem extends BaseEntity {
  ...
  @Column({ name: 'sales_unit_price', type: 'decimal', precision: 18, scale: 2, comment: '销售单价（USD）' })
  salesUnitPrice: string

  @Column({ name: 'sales_amount', type: 'decimal', precision: 18, scale: 2, comment: '销售金额（USD）' })
  salesAmount: string

  @Column({ name: 'total_cost', type: 'decimal', precision: 18, scale: 2, default: 0, comment: '产品总成本（FIFO 汇总）' })
  totalCost: string = '0'

  @Column({ name: 'gross_profit', type: 'decimal', precision: 18, scale: 2, default: 0, comment: '产品毛利润' })
