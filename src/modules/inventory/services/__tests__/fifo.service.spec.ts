...
describe('FifoService', () => {
  ...
  beforeEach(async () => {
    ...
    mockManager.create.mockImplementation((_: any, data: any) => ({ id: '9999999999999999', ...data }))
  ...
  describe('consume', () => {
    it('应从单个批次完全扣减', async () => {
      const batch = makeBatch({ availableQuantity: '100.0000', stockQuantity: '100.0000' })
      const inventory = makeInventory({ availableQuantity: '100.0000', stockQuantity: '100.0000' })
    ...
    it('应跨批次扣减（FIFO 顺序）', async () => {
      const batch1 = makeBatch({
        id: 'batch-1', batchNo: 'BT001',
        availableQuantity: '30.0000', stockQuantity: '30.0000',
        unitCost: '40.00', inboundTime: new Date('2026-01-01'),
      })
      const batch2 = makeBatch({
        id: 'batch-2', batchNo: 'BT002',
        availableQuantity: '70.0000', stockQuantity: '70.0000',
        unitCost: '60.00', inboundTime: new Date('2026-02-01'),
      })
      const inventory = makeInventory({ availableQuantity: '100.0000', stockQuantity: '100.0000' })
    ...
    it('库存不足时应抛出 BadRequestException', async () => {
      ...
      await expect(service.consume('prod-1', 50, 'biz-1'))
        .rejects.toThrow(BadRequestException)
    })

    it('扣减数量 <= 0 时应抛出 BadRequestException', async () => {
      await expect(service.consume('prod-1', 0, 'biz-1'))
        .rejects.toThrow(BadRequestException)

      await expect(service.consume('prod-1', -5, 'biz-1'))
        .rejects.toThrow(BadRequestException)
    })
    ...
    it('批次完全扣减后应标记 status=2（耗尽）', async () => {
      const batch = makeBatch({
        availableQuantity: '50.0000', stockQuantity: '50.0000', frozenQuantity: '0.0000',
      })
      const inventory = makeInventory({ availableQuantity: '50.0000', stockQuantity: '50.0000' })
  ...
  describe('freeze', () => {
    it('应正确冻结库存（available → frozen）', async () => {
      const batch = makeBatch({ availableQuantity: '100.0000', frozenQuantity: '0.0000' })
      const inventory = makeInventory({ availableQuantity: '100.0000', frozenQuantity: '0.0000' })
    ...
    it('可销售库存不足时应拒绝冻结', async () => {
      ...
      await expect(service.freeze('prod-1', 50, 'order-1'))
        .rejects.toThrow(BadRequestException)
    })

    it('冻结数量 <= 0 时应抛出异常', async () => {
      await expect(service.freeze('prod-1', 0, 'order-1'))
        .rejects.toThrow(BadRequestException)
    })

    it('跨批次冻结应按 FIFO 顺序', async () => {
      const batch1 = makeBatch({
        id: 'b1', availableQuantity: '20.0000', frozenQuantity: '0.0000',
        inboundTime: new Date('2026-01-01'),
      })
      const batch2 = makeBatch({
        id: 'b2', availableQuantity: '80.0000', frozenQuantity: '0.0000',
  ...
  describe('unfreeze', () => {
    it('应正确解冻库存（frozen → available）', async () => {
      const batch = makeBatch({
        availableQuantity: '70.0000', frozenQuantity: '30.0000', freezeStatus: 2,
      })
      const inventory = makeInventory({
        availableQuantity: '70.0000', frozenQuantity: '30.0000',
      })
    ...
    it('冻结库存不足时应拒绝解冻', async () => {
      ...
      await expect(service.unfreeze('prod-1', 50, 'order-1'))
        .rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe('deductFrozen', () => {
    it('应从冻结库存中扣减', async () => {
      const batch = makeBatch({
        availableQuantity: '70.0000', frozenQuantity: '30.0000', stockQuantity: '100.0000',
        unitCost: '50.00',
      })
      const inventory = makeInventory({
        availableQuantity: '70.0000', frozenQuantity: '30.0000', stockQuantity: '100.0000',
    ...
    it('冻结库存不足时应拒绝扣减', async () => {
      ...
      await expect(service.deductFrozen('prod-1', 50, 'shipment-1'))
        .rejects.toThrow(BadRequestException)
    })
  })
