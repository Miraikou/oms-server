...
export class InventoryBatch {
  ...
  @Column({ name: 'receipt_item_id', type: 'bigint', nullable: true, comment: '来源入库明细 ID（退货恢复/调整时可为空）' })
  receiptItemId: string | null = null

  @Index('idx_batch_source')
  @Column({ name: 'batch_source', type: 'tinyint', default: 1, comment: '批次来源：1=采购入库 2=退货恢复 3=库存调整' })
  batchSource: number = 1

  @Column({ name: 'batch_no', type: 'varchar', length: 50, comment: '批次号' })
  batchNo: string

  @Column({ name: 'unit_cost', type: 'decimal', precision: 18, scale: 2, comment: '批次采购单价' })
  unitCost: string

  @Column({ name: 'original_quantity', type: 'decimal', precision: 18, scale: 4, comment: '原始入库数量' })
  originalQuantity: string

  @Column({ name: 'available_quantity', type: 'decimal', precision: 18, scale: 4, comment: '当前可用数量' })
  availableQuantity: string

  @Column({ name: 'frozen_quantity', type: 'decimal', precision: 18, scale: 4, default: 0, comment: '当前冻结数量' })
  frozenQuantity: string = '0'

  @Column({ name: 'stock_quantity', type: 'decimal', precision: 18, scale: 4, comment: '当前库存（可用+冻结）' })
  ...
  @Column({ name: 'freeze_status', type: 'tinyint', default: 1, comment: '冻结状态：1=正常 2=部分冻结 3=全部冻结' })
