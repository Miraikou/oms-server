...
export class DictionaryService {
  ...
  /** 动态字典类型 → 数据源映射 */
  private readonly dynamicSources: Record<
    string,
    { repo: Repository<ObjectLiteral>; codeField: string; labelField: string }
  >;
  ...
  constructor(
    ...
    @InjectRepository(ExpressCompany)
    private readonly expressCompanyRepo: Repository<ExpressCompany>,
    @InjectRepository(TransportChannel)
    private readonly transportChannelRepo: Repository<TransportChannel>,
    ...
    this.dynamicSources = {
      COST_TYPE: {
        repo: this.costTypeRepo,
        codeField: 'id',
        labelField: 'costName',
      },
      EXPRESS_COMPANY: {
        repo: this.expressCompanyRepo,
        codeField: 'id',
        labelField: 'companyName',
      },
      TRANSPORT_CHANNEL: {
        repo: this.transportChannelRepo,
        codeField: 'id',
  ...
  async getDynamicDictionary(type: string): Promise<DynamicDictItem[]> {
    ...
    if (cached) {
      ...
      return JSON.parse(cached) as DynamicDictItem[];
    }
    ...
    const rawEntities: Array<Record<string, unknown>> = await repo
    ...
    const items: DynamicDictItem[] = rawEntities.map(
      (row: Record<string, unknown>) => ({
        code: String(row[`e_${codeField}`]),
        label: String(row[`e_${labelField}`]),
      }),
    );
